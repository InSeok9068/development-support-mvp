...
- 기본 원칙: Tailwind는 레이아웃(간격/정렬/반응형) 용도로만 사용한다.
- 기본 금지: 색/폰트/배경/보더/그림자/라운드 등 시각 스타일은 사용하지 않는다.
- 절대 금지:
  - Shoelace 컴포넌트의 기능/동작/시각 표현을 Tailwind로 대체하거나 강화하는 행위
  - 단순 미관 개선(더 예뻐 보이게 하기 위한 목적)만을 위한 Tailwind 클래스 추가
- 예외 허용(아래 3개만 허용, 이 범위 외는 금지):
  - 상태 표현 텍스트 색상(에러/성공/경고), 다크모드 대비 유지 필수
  - 레이아웃 보조 목적의 `border`/`rounded`/`shadow` (구획 분리, 카드 구분 등 기능적 목적)
  - 정보 위계 전달용 최소 타이포그래피(`text-sm`, `text-lg`, `font-semibold`)
- 판단 우선순위:
  1. Shoelace API/속성/슬롯/이벤트로 해결 가능한가?
  2. 불가능하면 Tailwind 레이아웃 유틸로 해결 가능한가?
  3. 그래도 필요하면 위 예외 허용 3개 범위에 해당하는가?
  4. 위 기준을 충족하지 않으면 Tailwind 스타일 적용 금지
...
- 테스트가 필요한 코드 수정의 경우, `pnpm lint`에 더해 `pnpm test`를 추가 실행한다.
...
4. 위 절차가 모두 완료된 경우에만 작업을 "완료"로 간주한다.

- `pnpm lint` 단계에서 오류가 발생한 경우, 작업은 완료된 것으로 간주하지 않는다.
