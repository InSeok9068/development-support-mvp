...
- 단, MVP 속도와 명확한 의미 전달을 위해 아래 최소 범위는 예외적으로 허용한다.
  - 상태 표현(에러/성공/경고)용 텍스트 색상 (다크모드에서도 대비가 유지되도록 고려)
  - 레이아웃 보조 목적의 border/rounded/shadow(구획 분리, 카드 구분 등 기능적 목적에 한함)
  - 정보 위계 전달을 위한 최소 타이포그래피(`text-sm`, `text-lg`, `font-semibold` 수준)
